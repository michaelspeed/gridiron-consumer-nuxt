<template>
  <section
    ...
    :style="{ 'margin-top': mainMobile ? '60px' : '0' }"
  >
    <div class="ads-group v2 nospc">
      <div class="container container-240">
        <div class="row" v-if="getHomePage.single.main">
          <div
            class="col-lg-3 col-md-3 col-sm-12 col-xs-12 set-w"
            v-if="!mainMobile"
          >
            <v-sheet
            ...
            >
              <client-only>
                <v-list subheader>
                  <v-list-item
                    v-for="child of GetCollectionTree"
                    :key="child.id"
                    v-if="child.name !== 'default'"
                    style="cursor: pointer"
                  >
                    <v-list-item-content
                      v-if="child.children.length === 0"
                      @click="$router.push(`/collection/${child.id}`)"
                    >
                    ...
                    >
                      ...
                      <v-card>
                        ...
                        <v-list
                          v-for="childcol of child.children"
                          :key="childcol.id"
                        >
                          <v-list-item
                            @click="$router.push(`/collection/${childcol.id}`)"
                          ...
                          >
                            <template v-slot:activator="{ on, attrs }">
                              <v-list-item
                                @click="
                                  $router.push(`/collection/${childcol.id}`)
                                "
                              >
                                <v-list-item-content>
                                  <v-list-item-title>{{
                            ...
                            <v-card>
                              ...
                              <v-list
                                v-for="subitem of childcol.children"
                                :key="subitem.id"
                              >
                                <v-list-item
                                  @click="
                                    $router.push(`/collection/${subitem.id}`)
                                  "
                                >
                                  <v-list-item-content>
                                    <v-list-item-title>{{
          ...
          <div class="col-lg-9 col-md-9 col-sm-12 col-xs-12 set-flex">
            <div class="slide-home3">
              <v-carousel
                ...
                continuous
                hide-delimiters
                :height="mainMobile ? '300px' : '500px'"
              >
                <v-carousel-item
                  v-for="(mainitems, index) of getHomePage.single.main"
                  @click="onClickCarousel(mainitems)"
                  :key="index"
                >
                  <div style="background-color: white;">
                    <a href="javascript:;" @click="onClickCarousel(mainitems)">
                      ...
                      <v-img
                        :src="`${assetLink}/${mainitems.preview.preview}`"
                        contain
                        :height="mainMobile ? 300 : 500"
                      ></v-img>
                    </a>
                    <v-sheet
                      class="slide-content v2"
                      style="padding: 20px"
                      color="primary"
                      elevation="2"
                    >
                      <h2
                        class="v2 font-weight-bold"
                        style="font-weight: bolder !important; color: white"
                        v-if="mainitems.type === 'product' && !mainMobile"
                      >
                        {{ mainitems.target.productName.substring(0, 30) }}
                      </h2>
                      <h6
                        class="v2 font-weight-bold"
                        style="font-weight: bolder !important; color: white"
                      ...
                      <h2
                        class="v2 font-weight-bold"
                        style="font-weight: bolder !important; color: white"
                        v-else-if="mainitems.type === 'variant' && !mainMobile"
                      >
                        {{ mainitems.target.name.substring(0, 30) }}
                      </h2>
                      <h6
                        class="v2 font-weight-bold"
                        style="font-weight: bolder !important; color: white"
                      ...
                      <h2
                        class="v2 font-weight-bold"
                        style="font-weight: bolder !important; color: white"
                        v-else-if="mainitems.type === 'category' && !mainMobile"
                      >
                        {{ mainitems.target.name.substring(0, 30) }}
                      </h2>
                      <h6
                      ...
                      <v-btn text color="white" :x-small="mainMobile"
                        >Shop Now</v-btn
                      >
                    </v-sheet>
                  </div>
    ...
    <div class="feature v2" v-if="!mainMobile">
      <div class="container container-240">
        <div class="feature-inside">
          ...
          <div class="feature-block v2 text-center">
            <div class="feature-block-img">
              <i class="fas fa-credit-card icon-mega"></i>
            </div>
            <div class="feature-info v2">
    ...
    <div class="feature-product spc2">
      <div class="container container-240">
        <client-only>
          <div
            v-for="(listitem, index) of getHomePage.single.lists"
            :class="{ active: index === listIndex }"
            :key="index"
          >
            <div class="ecome-heading style2" style="background-color: white">
            ...
            >
              <slide
                v-for="(sublistitem, subindex) of listitem.items"
                :key="subindex"
                style="height: 400px"
              >
...
import {
  GetCollectionTreeDocument, GetDefaultStoreDocument,
  GetHomePageDocument,
  GetMenuDocument, Store,
} from "~/gql";
...
@Component({
  ...
  async asyncData(context: any) {
    ...
    const getHomePage = await client
      .query({
        query: GetHomePageDocument,
      })
    const defstore = await client.query({query: GetDefaultStoreDocument})
    return {
      getHomePage: getHomePage.data.getHomePage,
      def: defstore.data.GetDefaultStore
    }
  },
})
